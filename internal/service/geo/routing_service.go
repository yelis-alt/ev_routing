...
func (rs *RoutingService) GetAdjacencyMatrix(routeRequest *dto.RouteRequestDTO) (map[int]map[int]dto.Edge, error) {
	...
	consumption := additional.SpecificConsumptionKWhPer100Km(routeRequest.Temperature, routeRequest.SpendOpt)
	...
	for idx, pair := range pairs {
		...
		tripDuration := additional.HoursToDuration(route.TripDuration)
		nearTariff := additional.NearestStationTariff(candidateStations, from.Coords, to.Coords)
		...
		if edge, ok := additional.BuildDirectedEdge(from, to, route.Distance, tripDuration, avgSpeedKmH, consumption, nearTariff, routeRequest); ok {
			adjacencyMatrix[from.Id][to.Id] = edge
		}
		if edge, ok := additional.BuildDirectedEdge(to, from, route.Distance, tripDuration, avgSpeedKmH, consumption, nearTariff, routeRequest); ok {
...
func buildOrderedStationList(routeRequest *dto.RouteRequestDTO) []dto.StationDTO {
	candidateStations := additional.FilterCandidateStations(routeRequest)

	startPoint := dto.StationDTO{Id: additional.StartStationId, Coords: routeRequest.StartCoords}
	finishPoint := dto.StationDTO{Id: additional.FinishStationId, Coords: routeRequest.FinishCoords}
...
// Shared by every search strategy in the algo package.
func GetRouteNodesFromIds(
	...
	previousId := additional.StartStationId
	...
	for _, station := range buildOrderedStationList(routeRequest) {
		...
		if stationId != additional.StartStationId {
